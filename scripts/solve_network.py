...
def prepare_network(n, solve_opts):
    ...
    if solve_opts.get('load_shedding'):
        if 'Load' in n.carriers.index:
            print('Load Shedding Feature is already activated')
        else:
...
def add_CCL_constraints(n, config):
    ...
    stor_country = n.storage_units.bus.map(n.buses.country)
    # cc means country and carrier
    if any(n.storage_units.p_nom_extendable == True):
        p_nom_per_cc_s = (pd.DataFrame(
                        {'p_nom': linexpr((1, get_var(n, 'StorageUnit', 'p_nom'))),
                        'country': stor_country, 'carrier': n.storage_units.carrier})
                        .dropna(subset=['p_nom'])
                        .groupby(['country', 'carrier']).p_nom
                        .apply(join_exprs))

    else:
        for i in list(agg_p_nom_minmax.index):
            if i[1] not in list(n.generators[n.generators.p_nom_extendable].carrier):
                logger.info("Droping {} from yearly constraints".format(i))
                agg_p_nom_minmax.drop(i, inplace=True)
    ...
    p_nom_per_cc = pd.concat([p_nom_per_cc_g, p_nom_per_cc_s], ignore_index=False)
